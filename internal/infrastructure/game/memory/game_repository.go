...
type InMemoryGameRepository struct {
	...
	roomToGame map[roomEntity.RoomID]entity.GameID
	mutex      sync.RWMutex
}

// NewInMemoryGameRepository creates a new in-memory game repository
func NewInMemoryGameRepository() repo.GameRepository {
	return &InMemoryGameRepository{
		games:      make(map[entity.GameID]*entity.Game),
		roomToGame: make(map[roomEntity.RoomID]entity.GameID),
	}
}
...
func (r *InMemoryGameRepository) GetGameByRoomID(roomID roomEntity.RoomID) (*entity.Game, error) {
	r.mutex.RLock()
	defer r.mutex.RUnlock()
